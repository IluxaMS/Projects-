...
void print_visual_char(unsigned char c, FILE *out) { // эта функция реализует флаг -v (visual). Её задача — превратить невидимые управляющие символы (коды 0-31, 127, >127) в читаемый текст (например, ^C, M-X).
  if (c == '\t') { // обработка табуляции (\t)
    fprintf(out, "^I"); // В нотации Unix Control-символы обозначаются как ^ + буква. Tab — это Control+I. 
      ...
      for (int j = 1; argv[i][j] != '\0'; j++) { // внутренний цикл (j): Перебирает символы внутри одного аргумента(это позволяет   поддерживать слитные флаги(например "-ne")), начинаем со второго символа, так как первый — это минус
        switch (argv[i][j]) {
          case 'b':
            ...
            break;
          case 'e': // важный нюанс ТЗ: Флаг -e подразумевает включение флага -v.
          ...
          case 't':
            flags->t = true;
            flags->v = true; // -t тоже подразумевает -v
            break;
          case 'T':
            flags->T_flag = true;
            flags->t = true; // -T не подразумевает -v
            break;
          case 'v':
...
static void handle_flag_e(char **argv, int *i, int argc, char **patterns,
                          int *count, bool *e_flag, bool *needs_free) {
    ...
    for (int i = file_start_idx; i < argc; i++) {
      ...
      if (!fp) {
        ...
        while (getline(&line, &len, fp) != -1) {
          cat_process_single_line(line, flags, &line_num, &non_empty_num,
                                  &prev_empty);
        }
        free(line);
        fclose(fp);
